...
namespace InternalHelper.Tests
{
    ...
    public class TestObject
    {
        ...
        /// </summary>
        /// -------------------------------------------------------------------
        static XmlDocument PatternsDocument = new XmlDocument();
        ...
        /// </summary>
        /// -------------------------------------------------------------------
        static XmlDocument ControlsDocument = new XmlDocument();
        ...
        /// </summary>
        /// ------------------------------------------------------------------------
        internal IUIAStructureChange _structChange = null;
            ...
            foreach (MethodInfo method in type.GetMethods())
            {
                foreach (Attribute attr in method.GetCustomAttributes(true))
                {
                    if (attr is TestCaseAttribute)
                    {
                        ...
                        if (testAttribute.Status.Equals(TestStatus.Works))
                        {
...
//                    throw new Exception("Steps count(" + m_TestStep + ") executed do not match of with the TestCaseAttribute.Description(" + testCaseAttribute.Description.Length + ")");
            ...
            catch (Exception exception)
            {
                ...
                UIAVerifyLogger.LogError(exception); // Logger will confirm if its a true/pass fail or not
                Passed = false;
            }
            try
            {
            ...
            catch (Exception exception)
            {
                ...
                UIAVerifyLogger.LogError(exception);
                Passed = false;
            }
            finally
            {
                ...
                if (Passed)
                {
                    UIAVerifyLogger.LogPass();
                }

                UIAVerifyLogger.EndTest();
            }
            return passed;
        }
        ...
        /// -------------------------------------------------------------------
        /// <summary>Wrapper the does UIAVerifyLogger.StartTest and various up
        ...
        private void StartTestWrapper(TestCaseAttribute testCaseAttribute, MethodInfo methodInfo)
        {
            if (m_le != null)
            {
                ...
                AutomationElementPathFromRoot apfr = new AutomationElementPathFromRoot(m_le);
                if (apfr != null)
                {
                    UIAVerifyLogger.StartTest(apfr.XmlNode, testCaseAttribute, methodInfo);
                }
                else
                {
                    UIAVerifyLogger.StartTest(null, testCaseAttribute, methodInfo);
                }
            }
            else
            {
        ...
        {
            //get the root logical element for the WPF window
        ...
        internal static string GetTestType(AutomationElement element)
        {
            ...
            return IDSStrings.IDS_NAMESPACE_UIAVERIFY + "." + IDSStrings.IDS_NAMESPACE_CONTROL + "." + cts + "ControlTests";
        ...
        static internal void Comment(string format, params object[] args)
        {
            ...
            UIAVerifyLogger.LogComment("{0}: {1}", m_TestStep, String.Format(format, args));
        }
        ...
        internal static string TestCaseCurrentStep
        {
            get
            {
                ...
                {
                    //Debug.Assert(false, "\"(m_TestCase != null) or (Step(" + m_TestStep + ") > descriptions(" + _testCaseAttribute.Description.GetLength(0) + "))\"");
                    return string.Empty;
                }
            }
        }
        ...
        static void LoadProxy(string proxySource, string proxyClass)
        {
            ...
            UIAVerifyLogger.LogComment("Loading proxy " + proxyClass + " from " + proxySource);
        ...
        static internal void ThrowMe(CheckType checkType, Exception exceptionThrown, string format, params object[] args)
        {
            ...
            if (
                _testCaseAttribute != null &&               /* may not be set if this is a pre test setup */
        ...
        internal void TSC_AddFocusChangedListener(string eventParamName, CheckType checkType)
        {
            ...
            // Allow the threading of UI Automation to do the focus change events 
            Thread.Sleep(100);
            m_TestStep++;
        }
        ...
        internal void TS_SupportsStructureChangeEvents(CheckType checkType)
        {
            ...
            if (!_appCommands.SupportsIUIAStructureChange(m_le))
        ...
        internal void TS_SupportsPropertyChangeEvents(CheckType checkType)
        {
            ...
            if (!_appCommands.SupportsIUIAPropertyChange(m_le))
        ...
        internal void TS_ResetControlToInitialState(AutomationElement element, CheckType checkType)
        {
            ...
            if (_structChange == null)
                _structChange = _appCommands.GetIUIAStructureChange();
        ...
        internal void TS_CausePropertyChange(AutomationElement element, AutomationProperty automationProperty, CheckType checkType)
        {
            ...
            IUIAPropertyChange pc = _appCommands.GetIUIAPropertyChange();
        ...
        internal void TSC_VerifyProperty(object actual, object expected, bool shouldBeEqual, object propName, CheckType checkType)
        {
            ...
            if (expected == null)
            {
                Comment("Verify that {0} for element ({2}) returns null", propName, shouldBeBuffer, Library.GetUISpyLook(m_le));
            }
            else
            {
                expectedBuffer = expected is string ? "\"" + expected.ToString() + "\"" : expected.ToString();
                Comment("Verify that {0} for element ({3}) {1} returns {2}", propName, shouldBeBuffer, expectedBuffer, Library.GetUISpyLook(m_le));
            }
            ...
            Comment("Element({0})'s property {1} is {2}", Library.GetUISpyLook(m_le), propName, actual);
            m_TestStep++;
        }
...
#pragma warning suppress 6517
        ...
        {
            Type typeThrown = actualException.GetType();
            if (typeThrown == expectedExceptionType)
            {
            ...
            {
                string erroStr = expectedExceptionType == null ? "none" : expectedExceptionType.ToString();
                if (string.IsNullOrEmpty(bugId))
                    ThrowMe(checkType, actualException, "The following expection was thrown incorrectly : " + typeThrown.FullName + " and expected: " + erroStr);
                else
                    ThrowMe(checkType, actualException, "The following expection was thrown incorrectly : " + typeThrown.FullName + " and expected: " + erroStr + ", bug id = " + bugId);
            }
        }
