...
namespace Microsoft.Test.UIAutomation.Tests.Patterns
{
    ...
    public sealed class AutomationElementTests : AutomationElementWrapper
    {
        ...
        /// <summary></summary>
        /// -------------------------------------------------------------------
        internal string argElement = "m_le";

        /// -------------------------------------------------------------------
        /// <summary></summary>
        /// -------------------------------------------------------------------
        Drawing.Color m_maskColor = Drawing.Color.FromArgb(0, 255, 0);

        /// -------------------------------------------------------------------
        /// <summary></summary>
        /// -------------------------------------------------------------------
        Drawing.Color m_textColor = Drawing.Color.FromArgb(0, 0, 255);
        ...
        [TestCaseAttribute("HwndWindowRect.1.UIA",
           TestSummary = "Verify UI Automation rectangle is correct for Hwnd based window",
        ...
        [TestCaseAttribute("HwndWindowRect.2.UIA",
           TestSummary = "Verify UI Automation rectangle available",
            Priority = TestPriorities.Pri0,
            Status = TestStatus.Works,
            Description = new string[] 
                {
                "Verification: Obtain AutomationElement.GetBoundingRectangle",
                })]
        public void HwndWindowRect2UIA(TestCaseAttribute testCase)
        {
            HeaderComment(testCase);

            // Get the UI Automation reported window rect
            Rect rcUIA = m_le.Current.BoundingRectangle;

            Comment("UI Automation BoundingRectangle returned (Left={0}, Top={1}, Right={2}, Bottom={3}", rcUIA.Left, rcUIA.Top, rcUIA.Right, rcUIA.Bottom);
            m_TestStep++;
        }
        ...
        [TestCaseAttribute("IsPasswordProperty.1.3.2",
            ...
            Description = new string[] 
                    ...
                    "Verify: Calling ValuePattern.Value throws an InvalidOperationException",
                    })]
            ...
            Description = new string[] 
                {
                    ...
                    "Verification: HelpTextProperty != null",
                    "Verification: HelpTextProperty != \"\"",
                    "Verification: HelpTextProperty != AutomationElement.NotSupported",
                })]
        ...
        [TestCaseAttribute("AutomationIdProperty.1.7.1",
            ...
            Description = new string[] 
            {
                "Precondition: This is not the RootElement and thus should have a parent",
                "Verify: No siblings have the same AutomationIdProperty"
            })]
        public void AutomationIdProperty171(TestCaseAttribute testCaseAttribute)
        {
            ...
            //"Precondition: This is not the RootElement and thus should have a parent",
            TSC_VerifyProperty(m_le, AutomationElement.RootElement, false, "AutomationElement.RootElement", CheckType.IncorrectElementConfiguration);
        ...
        [TestCaseAttribute("AutomationIdProperty.1.7.8",
            ...
            Description = new string[] 
            {
                ...
                "Precondition: Parent's ControlType != ListItem(known item that does not support AutomationIdProperty)",
                "Precondition: Parent's ControlType != DataItem(known item that does not support AutomationIdProperty)",
        ...
        public void AutomationIdProperty178(TestCaseAttribute testCaseAttribute)
        {
            ...
            bool testParentType = !Automation.Compare(m_le, AutomationElement.RootElement);
        ...
        ///<summary></summary>
        /// ---------------------------------------------------------------
        [TestCaseAttribute("GetRuntimeIdType.1",
            TestSummary = "Verify the format of the RuntimeId returned from the AutomationElement",
            Priority = TestPriorities.Pri0,
            Status = TestStatus.Works,
            Author = "Microsoft Corp.",
            Description = new string[] 
                {
                    "Verification: RuntimeId's should be of type int[]",
                })]
        public void RunteimId1(TestCaseAttribute testCaseAttribute)
        {
            HeaderComment(testCaseAttribute);

            object f = m_le.GetCurrentPropertyValue(AutomationElement.RuntimeIdProperty);

            // RuntimeId's should be of type int[]
            TSC_VerifyProperty(f.GetType(), typeof(int[]), "Type", CheckType.Verification);
        ...
        public void TestProperty1(TestCaseAttribute testCaseAttribute)
        {
            ...
            // ----------------------------------------------------------
            Comment("Do the AutomationProperties off of the AutomationElement");
            foreach (FieldInfo fi in assembly.GetType("System.Windows.Automation.AutomationElement").GetFields())
            {
                if (fi.FieldType == typeof(AutomationProperty))
                {
                    try
                    {
                        ...
                        Comment("Getting property value for (" + fi.Name + ")");
                        ...
                        Comment("Obtained GetCurrentPropertyValue(" + property.ProgrammaticName + ")");

                    }
                    catch (NoClickablePointException)
                    {
                        Comment("Eat NoClickablePointException, AutomationElementIdentifiers.ClickablePointProperty should throw this");
                    }
                    catch (NullReferenceException)
                    {
                        Comment("Eat NullReferenceException, AutomationElementIdentifiers.ClickablePointProperty should throw this");
                    }
                    catch (Exception exception)
                    {
                        Comment("Caught Exception: Exception ({0})", exception.GetType().ToString());
                        if (exception.InnerException != null)
                            Comment("Caught Exception: Exception.InnerException ({0})", exception.InnerException.GetType().ToString());
                        // Need to check inner exceptions since the reflection can hide the exception with reflection exceptions
                        if (!(exception.InnerException is NullReferenceException) &&
                            (!(exception.InnerException is NoClickablePointException))
                            )
                        {
                            ThrowMe(CheckType.Verification, string.Format("Failed Calling GetCurrentPropertyValue({0}) threw and Exception type({1}) with InnerException type({2})",
                                Helpers.GetProgrammaticName(property),
                                exception.GetType(),
            ...
            // ----------------------------------------------------------
            Comment("Do the AutomationProperties off of the Pattern objects");
        ...
        public void TestNavigation1(TestCaseAttribute testCaseAttribute)
        {
            ...
            AutomationElement nextPrevSibling;

            //"Precondition: AutomationElement's NextSibling != null",
            if (null == (nextSibling = TreeWalker.ControlViewWalker.GetNextSibling(m_le)))
                ThrowMe(CheckType.IncorrectElementConfiguration);
            m_TestStep++;

            //"Verify that the NextSibling.PreviousSibling != null",
            if (null == (nextPrevSibling = TreeWalker.ControlViewWalker.GetPreviousSibling(nextSibling)))
            ...
            if (!Automation.Compare(m_le, nextPrevSibling))
            {
                ThrowMe(CheckType.Verification,
                   string.Format("me({0})'s PreviousSibling({1}).NextSibling({2}) is not me",
                   Library.GetUISpyLook(m_le), Library.GetUISpyLook(nextSibling), Library.GetUISpyLook(nextPrevSibling)));
            }
                m_TestStep++;
        ...
        [TestCaseAttribute("Navigation.2",
            ...
            Description = new string[] {
                    ...
                    "Verify: AutomattinElement's Previous Sibling's Next Sibling != null",
                    "Verify that my PreviousSibling's NextSibling is me"
                })]
        public void TestNavigation2(TestCaseAttribute testCaseAttribute)
        {
            HeaderComment(testCaseAttribute);
            AutomationElement prevNextSibling = null;


            AutomationElement prevSibling = TreeWalker.ControlViewWalker.GetPreviousSibling(m_le);
            ...
            //"Verify: AutomattinElement's Previous Sibling's Next Sibling != null",
            if (null == (prevNextSibling = TreeWalker.ControlViewWalker.GetNextSibling(prevSibling)))
                ThrowMe(CheckType.Verification, "Controls previous sibling's next sibling is NULL and cannot be");
            m_TestStep++;

            // "Verify that my PreviousSibling's NextSibling is me"
            if (!Automation.Compare(m_le, prevNextSibling))
                ThrowMe(CheckType.Verification, "me({0}) != PreviousSibling.NextSibling({1})", Library.GetUISpyLook(m_le), Library.GetUISpyLook(prevNextSibling));
            m_TestStep++;
        }
        ...
        [TestCaseAttribute("Navigation.3",
            ...
            Description = new string[] {
                    ...
                    "Verify that my FirstChild's Parent != null",
                    "Verify that my FirstChild's Parent is me"
                })]
        public void TestNavigation3(TestCaseAttribute testCaseAttribute)
        {
            HeaderComment(testCaseAttribute);
            AutomationElement firstChild;
            AutomationElement parent;

            // "Precondition: AutomationElement's FirstChild != null", 
            if (null == (firstChild = TreeWalker.ControlViewWalker.GetFirstChild(m_le)))
                ThrowMe(CheckType.IncorrectElementConfiguration);
            m_TestStep++;

            // "Verify that my FirstChild's Parent != null",
            if (null == (parent = TreeWalker.ControlViewWalker.GetParent(firstChild)))
                ThrowMe(CheckType.Verification, "Element does not have a parent");
            m_TestStep++;

            // "Verify that my FirstChild's Parent is me"
            if (!Automation.Compare(m_le, parent))
                ThrowMe(CheckType.Verification,
                    string.Format("me({0})'s FirstChild({1}).Parent({2}) is not me",
                    Library.GetUISpyLook(m_le), Library.GetUISpyLook(firstChild), Library.GetUISpyLook(parent)));
            m_TestStep++;
        }
        ...
        [TestCaseAttribute("Navigation.4",
            ...
            Description = new string[] {
                    ...
                    "Verify that my LastChild's Parent != null",
        ...
        public void TestNavigation4(TestCaseAttribute testCaseAttribute)
        {
            ...
            AutomationElement lastChild;
            AutomationElement parent;

            // "Precondition: AutomationElement's LastChild != null", 
            if (null == (lastChild = TreeWalker.ControlViewWalker.GetLastChild(m_le)))
                ThrowMe(CheckType.IncorrectElementConfiguration);
            m_TestStep++;

            // "Verify that my LastChild's Parent != null",
            if (null == (parent = TreeWalker.ControlViewWalker.GetParent(lastChild)))
                ThrowMe(CheckType.Verification, "Element does not have a parent");
            m_TestStep++;
            ...
            if (!Automation.Compare(m_le, parent))
                ThrowMe(CheckType.Verification,
                    string.Format("me({0})'s LastChild({1}).Parent({2}) is not me",
                    Library.GetUISpyLook(m_le), Library.GetUISpyLook(lastChild), Library.GetUISpyLook(parent)));
            m_TestStep++;
        }
        ...
        [TestCaseAttribute("Navigation.7",
            ...
            Author = "Microsoft Corp.",
            Description = new string[] {
                    "Precondition: This is not the RootElement and thus has a parent",
        ...
        [TestCaseAttribute("SetFocus.1",
            ...
            Description = new string[] {
                ...
                "Precondition: Verify that the object's IsFocusableProperty is still true",
        ...
        public void TestSetFocus1a(TestCaseAttribute testCaseAttribute)
        {
            ...
            TS_RandomSetFocusToOtherControl(m_le, CheckType.IncorrectElementConfiguration);
        ...
        private void TS_Focused(AutomationElement m_le, bool shouldbe, CheckType checkType)
        {
            ...
            if (!Automation.Compare(m_le, AutomationElement.FocusedElement))
            {
        ...
        ///<summary></summary>
        /// ------------------------------------------------------------------------
        [TestCaseAttribute("SetFocus.10",
            TestSummary = "Setting focus to list that has no items selected should fire a focus change event",
            Priority = TestPriorities.Pri1,
            Status = TestStatus.Works,
            TestCaseType = TestCaseType.Events,
            EventTested = "AutomationFocusChangedEventHandler()",
            Author = "Microsoft Corp.",
            Description = new string[] {
                "Precondition: ControlType is ControlType.ListBox",
                "Precondition: Verify that the element's IsOffScreen = false",
                "Precondition: IsSelectionRequired = false",
                "Precondition: Verify that the object's IsKeyboardFocusableProperty == true",
                "Precondition: Make sure that there are no ListItems that are selected",
                "Step: Ensure the AutomationElement does not have focus by calling SetFocus(AutomationElement.RootElement.FirstChild)",
                "Step: Verify that the AutomationElement's HasKeyboardFocusProperty == false",
                "Step: Add FocusChangedListener event on the AutomationElement",
                "Step: Call and verify that SetFocus(AutomationElement) returns true",
        ...
        public void TestSetFocus10(TestCaseAttribute testCaseAttribute)
        {
            HeaderComment(testCaseAttribute);

            //"Precondition: ControlType is ControlType.ListBox",
            TSC_VerifyProperty(m_le.Current.ControlType, ControlType.List, "ControlType", CheckType.IncorrectElementConfiguration);

            //"Precondition: Verify that the element's IsOffScreen = false",
            TSC_VerifyProperty(m_le.Current.IsOffscreen, false, "Current.IsOffscreen", CheckType.IncorrectElementConfiguration);

            //"Precondition: IsSelectionRequired = false",
            TSC_VerifyProperty((bool)m_le.GetCurrentPropertyValue(SelectionPattern.IsSelectionRequiredProperty), false, "IsSelectionRequiredProperty", CheckType.IncorrectElementConfiguration);
        ...
        public void TestClickablePointProperty2(TestCaseAttribute testCaseAttribute)
        {
            try
            {
                ...
                {
                    object obj = m_le.GetCurrentPropertyValue(AutomationElement.ClickablePointProperty, true);
                    ...
                    if (m_le.Current.IsOffscreen)
                    {
                        if (!double.IsNaN(pt.X) || !double.IsNaN(pt.Y))
                            ThrowMe(CheckType.Verification, "Element's who's IsOffscreen property == true, should return NaN for ClickablePointProperty");
                    }
            ...
            {
                // Eat this as it may not have a clickable point
            }
            catch (NoClickablePointException)
            {
                // Eat this as it may not have a clickable point
            ...
            {
                ThrowMe(CheckType.KnownProductIssue, "This is a known bug");
            }
            m_TestStep++;
        }
        ...
        public void TestTryClickablePoint1(TestCaseAttribute testCaseAttribute)
        {
            ...
            Rect LERc = m_le.Current.BoundingRectangle;
            Point pt = new Point(-1, -1);
            Point UIAVerifyFoundPr = new Point(-1, -1);
            double ParentLeft = LERc.Left;
            double ParentTop = LERc.Top;
            ...
            // Step: Find a clickable point from this array
            TS_FindFirstClickablePoint(m_le, ref availableLocations, ref UIAVerifyFoundPr);

            Point pnt = new Point(-1, -1);

            if (UIAVerifyFoundPr.X.Equals(double.NaN) && UIAVerifyFoundPr.Y.Equals(double.NaN))
            {
            ...
            {
                UIAVerifyFoundPr.X += LERc.Left;
                UIAVerifyFoundPr.Y += LERc.Top;
                if (m_le.TryGetClickablePoint(out pnt).Equals(false))
                {
                    Comment("InternalHelper calculated  point: AutomationElement.FromPoint(" + UIAVerifyFoundPr + ") returned " + Library.GetUISpyLook(AutomationElement.FromPoint(UIAVerifyFoundPr)));
                    ...
                    ThrowMe(CheckType.Verification, "TryGetClickablePoint() returned false even through there was a point at " + UIAVerifyFoundPr + ".  TryGetClickablePoint returned the point(" + pnt + ")");
                }
            }
            m_TestStep++;
        }
        ...
        ///<summary></summary>
        /// ---------------------------------------------------------------
        private void TS_VerifyControlPersistsAfterLoosesFocus(AutomationElement element, CheckType checkType)
        {
            ControlType ct = element.Current.ControlType;

            // short curcuit when there is not parent
            if (Automation.Compare(element, AutomationElement.RootElement))
            {
                ThrowMe(checkType, "Element is the root element");
            }
            else
            {
                if (ct == ControlType.Edit)
                    if (TreeWalker.ControlViewWalker.GetParent(element).Current.ControlType == ControlType.DataItem)
