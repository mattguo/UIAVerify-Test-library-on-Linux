...
namespace Microsoft.Test.UIAutomation.Tests.Patterns
{
    ...
    public sealed class SelectionTests : SelectionPatternWrapper
    {
        ...
        void SupportsMultipleSelectionProperty_Test1(bool selectionRequired, bool supportMultipleSelection)
        {
            ...
            // 8: Call AddElementToSelection() on the second element and verify the success/failure of the call matches up with SupportsMultipleSelectionProperty
                            ...
                            case false: //AddElementToSelection(true) : CanSelectMultiple(false)
                                {
                                    ...
                                    {
                                        case 1:
                                            // $TODO (waiting on decision on this) ThrowMe("There is only one item selected after calling AddElementToSelection() with one element already selected, control's CanSelectMultiple = false...AddElementToSelection() most likley returned true incorrectly", CheckType.Verification);
                                            break;
                                        case 2:
