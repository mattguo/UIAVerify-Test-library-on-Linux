...
namespace Microsoft.Test.UIAutomation.Core
{
    ...
    public class Library
    {
        ...
        {
            /// ---------------------------------------------------------------
            /// <summary>Standard proxies that are shipped with UIA</summary>
            /// ---------------------------------------------------------------
            Default,
        ...
        static public void StartApplication(string appPath, string arguments, out IntPtr handle, out AutomationElement element, out Process process)
        {
            ...
            UIAVerifyLogger.LogComment("{0}Starting({1})", PREBUFFER, appPath);
...
//            element = AutomationElement.FromHandle(handle);
        ...
        {
            UIAVerifyLogger.LogComment("{0}Closing {1}", PREBUFFER, processName);
            ...
            {
                process.Kill();
                UIAVerifyLogger.LogComment("{0}{1} has been terminated", PREBUFFER, processName);
            }
            catch (InvalidOperationException)
            {
                UIAVerifyLogger.LogComment("{0}{1} has already been terminated", PREBUFFER, processName);
            }
        }
        ...
        static public string GetUISpyLook(AutomationElement le)
        {
            ...
            try
            {
                buffer += "\"" + le.Current.ControlType.ProgrammaticName.Replace("ControlType.", "") + "\" ";
            }
